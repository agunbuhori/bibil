...
@section('content')
  ...
  @slot('title') Mapel @endslot
  @slot('subtitle') Manage Mapel @endslot
  @endcomponent
      ...
      <div class="box-header with-border">
        ...
        <div class="box-tools pull-right" style="margin-top:0px;">
          <button type="button" href="#" class="btn btn-sm bg-blue"
            data-toggle="modal" data-target="#mapel" onclick="clearform()">
...
@section('javascript')
  <script>
    function clearform() {
      var modal = $('#mapel'),
          form  = modal.find('form[name="mapel"]')

      modal.find('.modal-title').text('Tambah Mapel')
      form.find('[name]').not('[name="_token"]').val('')
      form.attr('action', '{{ route('mapel.store') }}')
    }
    ...
    $('form[name="mapel"]').on('submit', function(e) {
      var form   = $(this),
          modal  = $('#mapel'),
    ...
    function edit(id) {
      ...
      modal.find('.modal-title').text('Ubah Mapel')
      form.attr('action', '{{ route('mapel.index') }}/' + id)
